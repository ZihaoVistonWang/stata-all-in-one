...
function getDataViewerFrameHtml(webview) {
...
<head>
    <meta charset="UTF-8">
    <meta http-equiv="Content-Security-Policy" content="default-src 'none'; font-src ${webview.cspSource} data:; style-src 'unsafe-inline'; script-src 'nonce-${nonce}';">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${escHtml(msg('dataViewerPanelTitle'))}</title>
    <style>
        :root {
            color-scheme: light dark;
            --vscode-editor-background: ${escHtml(themeVars.background)};
            --vscode-editor-foreground: ${escHtml(themeVars.foreground)};
            --vscode-foreground: ${escHtml(themeVars.foreground)};
            --vscode-descriptionForeground: ${escHtml(themeVars.description)};
            --vscode-panel-border: ${escHtml(themeVars.border)};
...
<body class="loading">
    ...
    <script nonce="${nonce}">
        const vscode = { postMessage: message => window.parent.postMessage({ __saioFrame: 'dataViewer', message }, '*') };
        let currentTab = 'vars';
        ...
        function applyHostTheme(message) {
            var variables = message && message.variables;
        ...
        var dataViewerInitialized = false;
        function initializeDataViewerFrame() {
            if (dataViewerInitialized) return;
            dataViewerInitialized = true;
            requestAnimationFrame(function () {
                syncFilterUi();
...
module.exports = { getDataViewerFrameHtml };
