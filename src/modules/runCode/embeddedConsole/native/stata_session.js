...
function initSession(dylibPath, splash = false, execPath = '', stHome = '') {
    // On Windows the C++ InitSession creates a dedicated thread, runs
    // StataSO_Main on it, and blocks until init completes.  On macOS
    // StataSO_Main runs synchronously on the calling thread.
 ...
 * g_StataSO_SetBreak is thread-safe — can be called from any thread
 * to interrupt the running StataSO_Execute.
 * @returns {void}
 */
function setBreak() {
    if (!isNativeLoaded()) {
