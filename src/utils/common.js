...
const UI_TEXT = {
    en: {
        ...
        cliUnavailable: ({ reason }) => `Webview execution unavailable, switched to GUI. Reason: ${reason}`,
        cliOfferGuiFallback: ({ reason }) => `Webview execution hit an extension error: ${reason}`,
        useStataApp: 'Use Stata App',
        stayInCli: 'Stay in Webview',
        sideLeft: 'left',
        sideRight: 'right',
    ...
    zh: {
        ...
        cliUnavailable: ({ reason }) => `Webview 执行不可用，已切换到 GUI。原因：${reason}`,
        cliOfferGuiFallback: ({ reason }) => `Webview 执行遇到扩展错误：${reason}`,
        useStataApp: '使用 Stata App',
        stayInCli: '留在 Webview',
        sideLeft: '左侧',
        sideRight: '右侧',
