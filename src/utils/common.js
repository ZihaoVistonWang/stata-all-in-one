...
const UI_TEXT = {
    en: {
        ...
        webviewOverflowNotice: 'Output is wider than the current view. Scroll horizontally to inspect it, or widen the Embedded Console panel for a better fit.',
        secondarySidebarOverflowNotice: 'Output is wider than the current view. Scroll horizontally to inspect it, or widen the secondary sidebar for a better fit.',
        embeddedConsoleSurfaceName: 'Embedded Console',
        secondarySidebarConsoleSurfaceName: 'Secondary Sidebar Console',
        consoleOverflowNoticeReset: ({ surface }) => `${surface} overflow notice reset. It will show again when output overflows.`,
        consoleOverflowNoticeResetExternal: 'External App mode does not use the console overflow notice. The reset will apply after switching to a console run mode.',
        ...
        secondarySidebarConsoleTab: 'Console',
        secondarySidebarDataViewerTab: 'Data Viewer',
        ...
        dataViewerOpenedSidebarNotice: 'Opened Data Viewer in the SAiO secondary sidebar.',
        ...
        aiSkillWelcomePrompt: 'I have Stata All in One installed. Please install and use its bundled native Stata AI Skill v1.1 so you can run Stata code:\n\n1. Find the installed extension folder. Search under ~/.vscode/extensions/ or ~/.vscode-insiders/extensions/ for a folder matching *stata-all-in-one*. On Windows, search under %USERPROFILE%\\.vscode\\extensions.\n2. Install or register <extension-folder>/skill as an AI Agent skill using your platform mechanism, such as a skills directory, rules file, connector config, or equivalent. The skill name must be exactly "stata-ai-skill". Do not rename it.\n3. Open <extension-folder>/skill/SKILL.md and treat it as the sole authoritative definition for executable resolution, service startup, setup, reset, reconfiguration, and HTTP usage. Follow its Agent Workflow instead of using an older or generic Stata setup procedure.\n4. The default Stata AI Skill endpoint is http://127.0.0.1:19522. Check /status first and follow setup.phase exactly, including explicit detected-candidate selection or the documented two-stage aiskill setup fallback when required.\n5. For reset or reconfiguration requests, follow the reset workflow in SKILL.md rather than overwriting the persisted selection directly.\n6. If you run in a container, dev container, remote shell, or sandbox, remember that 127.0.0.1 may refer to the sandbox instead of my host machine. Test /status with a short timeout and, if it fails, ask for host-side command execution or tell me exactly which command to run in a host terminal.',
    ...
    zh: {
        ...
        webviewOverflowNotice: '输出内容超出了当前宽度，请横向滑动查看，或拉宽嵌入式控制台面板以更好地适配输出。',
        secondarySidebarOverflowNotice: '输出内容超出了当前宽度，请横向滑动查看，或拉宽辅助侧栏以更好地适配输出。',
        embeddedConsoleSurfaceName: '嵌入式控制台',
        secondarySidebarConsoleSurfaceName: '辅助侧栏控制台',
        consoleOverflowNoticeReset: ({ surface }) => `${surface}的输出过宽提示已重置。当输出再次超出当前宽度时，该提示将重新显示。`,
        consoleOverflowNoticeResetExternal: '当前为外部 Stata 应用模式，不使用控制台输出过宽提示。重置将在切换到控制台运行模式后生效。',
        ...
        secondarySidebarConsoleTab: '控制台',
        secondarySidebarDataViewerTab: '数据查看器',
        ...
        dataViewerOpenedSidebarNotice: '已在 SAiO 辅助侧栏中打开数据查看器。',
        ...
        aiSkillWelcomePrompt: '我已经安装了 Stata All in One。请安装并使用它内置的原生 Stata AI Skill v1.1，以便运行 Stata 代码：\n\n1. 找到已安装的扩展目录。在 ~/.vscode/extensions/ 或 ~/.vscode-insiders/extensions/ 下搜索匹配 *stata-all-in-one* 的文件夹；Windows 在 %USERPROFILE%\\.vscode\\extensions 下搜索。\n2. 按你的平台机制把 <扩展目录>/skill 安装或注册为 AI Agent skill，例如 skills 目录、rules 文件、connector config 或同类配置。skill 名称必须严格为 "stata-ai-skill"，不要改名。\n3. 打开 <扩展目录>/skill/SKILL.md，将它作为解析二进制、启动服务、初始配置、重置、重新配置和 HTTP 调用的唯一权威定义。请遵循其 Agent Workflow，不要使用旧版或通用的 Stata 配置流程。\n4. Stata AI Skill 默认端点是 http://127.0.0.1:19522。先检查 /status，再严格按 setup.phase 处理，包括明确确认自动发现的 Stata 候选项，或在需要时使用文档中的两阶段 aiskill setup 后备流程。\n5. 当我要求重置或重新配置时，按 SKILL.md 中的 reset 流程执行，不要直接覆盖已保存的 Stata 选择。\n6. 如果你运行在容器、Dev Container、远程 shell 或沙箱中，要注意 127.0.0.1 可能指向沙箱而不是我的宿主机。请用短超时测试 /status；如果失败，请申请宿主机命令执行权限，或明确告诉我需要在宿主机终端运行哪条命令。',
