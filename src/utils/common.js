...
const UI_TEXT = {
    en: {
        ...
        customSepPrompt: 'Enter one separator character. Leave empty to use spaces for centering a heading.',
        customSepPlaceholder: 'Leave empty for spaces',
        ...
        webviewStopping: 'Stopping Stata execution...',
        webviewRestarting: 'Restarting Stata session...',
        webviewIdle: 'Idle',
        webviewError: 'Error',
        webviewStop: 'Stop',
        webviewJumpToBottom: 'Jump to bottom',
        consoleRunNavigation: 'Run navigation',
        consoleExpandInput: 'Expand all',
        consoleCollapseInput: 'Collapse',
        consoleExpandResult: 'Expand result',
        consoleResultPreviewTitle: 'Result Preview | Stata All in One',
        consoleForceStopped: 'Stata did not respond to Break. Its isolated session was terminated and will restart on the next run.',
        webviewClear: 'Clear and restart Stata session',
        consoleRestartFailed: ({ error }) => `Failed to restart the Stata session: ${error}`,
        consoleOpenFile: 'Open file',
        consoleFileNotFound: ({ filePath }) => `File not found: ${filePath}`,
        consoleFileOpenFailed: ({ filePath, error }) => `Failed to open ${filePath}: ${error}`,
        consoleSystemOpenRejected: 'The system did not accept the open request.',
        consoleSystemDefaultApplication: 'system default application',
        consoleOpeningWithExternalApplication: ({ appName }) => `Opening with ${appName}. Please wait...`,
        consoleExport: 'Export console results',
        consoleExportFormatPlaceholder: 'Select console export format',
        consoleExportHtml: 'HTML',
        consoleExportHtmlDescription: 'Self-contained offline result page',
        consoleExportMarkdown: 'Markdown',
        consoleExportMarkdownDescription: 'Single Markdown file with embedded images',
        consoleExportNotebook: 'Jupyter Notebook',
        consoleExportNotebookDescription: 'nbstata-compatible .ipynb file',
        consoleExportFileFilter: 'Console Export',
        consoleExportProgress: 'Exporting console results...',
        consoleExportSuccess: ({ fileName }) => `Console results exported to ${fileName}.`,
        consoleExportFailed: ({ message }) => `Failed to export console results: ${message}`,
        consoleExportNoHistory: 'There are no console results to export.',
        consoleExportWhileRunning: 'Wait for the current Stata run to finish before exporting.',
        consoleExportMissingGraphs: ({ count }) => `The export was saved, but ${count} graph${count === 1 ? '' : 's'} could no longer be read and were replaced with placeholders.`,
        consoleExportSourceBefore: 'This result file was exported by ',
        consoleExportSourceAfter: '.',
        consoleExportGraphUnavailable: ({ graphName }) => `[Graph unavailable: ${graphName}]`,
        ...
        composerTips: [
            ...
            'Tip: Click the AI button to copy the online Stata AI Skill installation prompt.',
        ...
        webviewOverflowNoticeWithPreview: 'Output is wider than the current view. Hover over the result and select Expand result, scroll horizontally, or widen the Console panel.',
        ...
        dataViewerDoubleClickCopyHint: 'Double-click a cell to copy its content',
        dataViewerAutoFitColumn: 'Double-click to fit column width',
        dataViewerCellCopied: ({ column, value }) => `Copied ${column}: ${value}`,
        dataViewerCellCopyFailed: ({ error }) => `Failed to copy cell content${error ? `: ${error}` : '.'}`,
        dataViewerObs: 'Obs',
        dataViewerVars: 'Vars',
        ...
        dataViewerVariableFilterPlaceholder: 'varlist',
        dataViewerNoVariableMatches: 'No matching variables',
        dataViewerFilterRequiresIf: ({ expression }) => `Invalid filter "${expression}". Add "if" before the condition, for example: if foreign == 1 & price < 10000.`,
        pinyinMatchingSlow: ({ duration }) => `Pinyin conversion for variable names and labels took about ${duration} ms and may affect typing responsiveness. Disable pinyin fuzzy matching for variables? Other Chinese, English, and character-based matching will remain available.`,
        pinyinMatchingDisableYes: 'Yes',
        pinyinMatchingDisableNo: 'No',
        dataViewerOpenedNotice: ({ title }) => `Opened in the "${title}" tab.`,
        consoleBusyAction: 'Stata is running. Please wait until it finishes before starting another action.',
        dataViewerUnsupportedFilter: ({ expression }) => `This filter is not supported for direct .dta viewing: ${expression}`,
        dataViewerDirectReadFailed: 'Unable to read the current Stata dataset directly.',
        dataViewerPluginLoadFailed: 'Unable to load the Stata data reader.',
        dataViewerPluginMissing: ({ path }) => `The Stata data reader plugin is missing: ${path}. Rebuild it with scripts/build-data-plugin.sh (macOS) or scripts/build-data-plugin.ps1 (Windows).`,
        dataViewerInvalidBuffer: 'The Stata data reader returned invalid data.',
        dataViewerMetadataMismatch: 'The Stata data and variable metadata do not match.',
        dataViewerMetadataMissing: 'Stata did not return dataset metadata. The dataset may have changed while it was being read.',
        dataViewerReadFailed: 'Unable to read the current dataset.',
        dataViewerReadCancelled: 'The data read was cancelled.',
        dataViewerCancelRead: 'Cancel',
        dataViewerCaptureTooLarge: ({ rows, megabytes, requiredMb }) => `Reading ${rows} observations at once needs about ${requiredMb} MB, which exceeds the Data Viewer memory limit of ${megabytes} MB. Raise "stata-all-in-one.dataViewerMemoryLimitMB" (currently ${megabytes}) to read it in one go, or filter the data first (for example: br if year == 2020).`,
        dataViewerZeroObservations: 'This dataset has no observations (0 rows), but its variables are shown.',
        dataViewerNoMatches: 'No observations match the current filter.',
        dataViewerOpenedButNotUpdated: 'The viewer was opened, but the data could not be read. The table still shows the previous data.',
        dataViewerOpenedWithError: ({ error }) => `The Data Viewer opened, but reading the data failed: ${error}`, 
        dataViewerTruncatedBuffer: 'The Stata data reader returned a truncated response ({what}).',
        dataViewerSessionUnavailable: 'The Stata session is no longer available. Run the data command again.',
        dataViewerSessionLost: 'The Stata session was lost (the Stata engine stopped or was force-stopped). Reload the data to continue; the table below is the previous data.',
        consoleUnsupportedCommand: ({ command }) => `The Embedded Console cannot run the External Stata command "${command}".`,
        consoleUnsupportedWhich: ({ command }) => `The Embedded Console cannot run the External Stata command "which ${command}".`,
        aiSkillDisabled: 'Stata AI Skill must be installed in your AI tool before use.',
        aiSkillSessionNotReady: 'Stata AI Skill service is not ready. Install the skill in your AI tool and start the native service.',
        ...
        aiSkillWelcomeIntro: 'Stata AI Skill is installed separately and lets AI coding tools run Stata through a native local service.',
        aiSkillWelcomeHint: 'Click "Copy Prompt" and paste it into Claude Code, Codex, Open Code, OpenClaw, or another AI coding tool. The prompt provides two online installation-guide addresses.',
        aiSkillWelcomePrompt: 'Fetch these two URLs at the same time, in parallel\n- https://raw.giteeusercontent.com/ZihaoVistonWang/Stata-AI-Skill/raw/main/guide/installation.md\n- https://raw.githubusercontent.com/ZihaoVistonWang/Stata-AI-Skill/refs/heads/main/guide/installation.md\n\nThey are identical mirrors of the same guide, one hosted domestically and one overseas. Use whichever responds first; do not wait for the other one to finish.\n\nFollow `installation.md` to install and configure Stata AI Skill.\n\nAfter installation, strictly follow the installed Skill\'s `SKILL.md` for service startup, status checks, Stata configuration, and subsequent operations. Do not use an outdated or generic Stata setup procedure.',
        ...
        capabilityUnverifiedAI: 'Click the AI button to copy the online Stata AI Skill installation prompt.',
    ...
    zh: {
        ...
        customSepPrompt: '输入一个分隔符字符；留空时使用空格将标题文本居中。',
        customSepPlaceholder: '留空使用空格',
        noEditor: '未找到活动编辑器',
        ...
        webviewStopping: '正在停止 Stata 任务...',
        webviewRestarting: '正在重启 Stata Session...',
        webviewIdle: '空闲',
        webviewError: '错误',
        webviewStop: '停止',
        webviewJumpToBottom: '跳转到底部',
        consoleRunNavigation: '运行记录导航',
        consoleExpandInput: '展开全部',
        consoleCollapseInput: '收起',
        consoleExpandResult: '展开结果',
        consoleResultPreviewTitle: '结果预览 | Stata All in One',
        consoleForceStopped: 'Stata 未响应中断请求，已终止独立会话；下次运行时将自动重启。',
        webviewClear: '清空并重启 Stata Session',
        consoleRestartFailed: ({ error }) => `重启 Stata Session 失败：${error}`,
        consoleOpenFile: '打开文件',
        consoleFileNotFound: ({ filePath }) => `文件不存在：${filePath}`,
        consoleFileOpenFailed: ({ filePath, error }) => `无法打开 ${filePath}：${error}`,
        consoleSystemOpenRejected: '系统未接受打开文件请求。',
        consoleSystemDefaultApplication: '系统默认程序',
        consoleOpeningWithExternalApplication: ({ appName }) => `正在使用${appName}打开，请等待...`,
        consoleExport: '导出控制台结果',
        consoleExportFormatPlaceholder: '选择控制台导出格式',
        consoleExportHtml: 'HTML',
        consoleExportHtmlDescription: '可离线打开的单文件结果页',
        consoleExportMarkdown: 'Markdown',
        consoleExportMarkdownDescription: '内嵌图片的单个 Markdown 文件',
        consoleExportNotebook: 'Jupyter Notebook',
        consoleExportNotebookDescription: '兼容 nbstata 的 .ipynb 文件',
        consoleExportFileFilter: '控制台导出文件',
        consoleExportProgress: '正在导出控制台结果...',
        consoleExportSuccess: ({ fileName }) => `控制台结果已导出至 ${fileName}。`,
        consoleExportFailed: ({ message }) => `导出控制台结果失败：${message}`,
        consoleExportNoHistory: '当前没有可导出的控制台结果。',
        consoleExportWhileRunning: '请等待当前 Stata 运行完成后再导出。',
        consoleExportMissingGraphs: ({ count }) => `文件已保存，但有 ${count} 张图形已无法读取，已在原位置改为占位说明。`,
        consoleExportSourceBefore: '本结果文件由 ',
        consoleExportSourceAfter: ' 导出。',
        consoleExportGraphUnavailable: ({ graphName }) => `[图形不可用：${graphName}]`,
        ...
        composerTips: [
            ...
            '提示：点击 AI 按钮可复制 Stata AI Skill 在线安装提示词。',
            '提示：嵌入式控制台和原生 Stata AI Skill 都需要有效的 STATA.LIC 许可证文件。',
        ],
        webviewOverflowNotice: '输出内容超出了当前宽度，请横向滑动查看。也可以拉宽 Console 面板以更好地适配输出。',
        webviewOverflowNoticeWithPreview: '输出内容超出了当前宽度。可将鼠标悬停在结果上并选择“展开结果”，也可横向滑动或拉宽 Console 面板。',
        ...
        dataViewerDoubleClickCopyHint: '双击单元格可复制内容',
        dataViewerAutoFitColumn: '双击自动调整列宽',
        dataViewerCellCopied: ({ column, value }) => `已复制${column}：${value}`,
        dataViewerCellCopyFailed: ({ error }) => `复制单元格内容失败${error ? `：${error}` : '。'}`,
        dataViewerObs: '观测值',
        dataViewerVars: '变量数',
        ...
        dataViewerVariableFilterPlaceholder: '变量列表',
        dataViewerNoVariableMatches: '没有匹配的变量',
        dataViewerFilterRequiresIf: ({ expression }) => `筛选条件“${expression}”不合法。请在条件前加上“if”，例如：if foreign == 1 & price < 10000。`,
        pinyinMatchingSlow: ({ duration }) => `变量名和变量标签的拼音转换本次耗时约 ${duration} 毫秒，可能影响输入流畅度。是否关闭变量拼音模糊匹配？普通中文、英文和字符模糊匹配不受影响。`,
        pinyinMatchingDisableYes: '是',
        pinyinMatchingDisableNo: '否',
        dataViewerOpenedNotice: ({ title }) => `已在“${title}”选项卡中打开。`,
        consoleBusyAction: 'Stata 正在运行，请等待运行结束后再操作。',
        dataViewerUnsupportedFilter: ({ expression }) => `直接查看 .dta 时暂不支持此筛选条件：${expression}`,
        dataViewerDirectReadFailed: '无法直接读取当前 Stata 数据集。',
        dataViewerPluginLoadFailed: '无法加载 Stata 数据读取器。',
        dataViewerPluginMissing: ({ path }) => `找不到 Stata 数据读取器插件：${path}。请用 scripts/build-data-plugin.sh（macOS）或 scripts/build-data-plugin.ps1（Windows）重新编译。`,
        dataViewerInvalidBuffer: 'Stata 数据读取器返回了无效数据。',
        dataViewerMetadataMismatch: 'Stata 数据与变量信息不匹配。',
        dataViewerMetadataMissing: 'Stata 未返回数据集元数据。读取过程中数据集可能已发生变化。',
        dataViewerReadFailed: '无法读取当前数据集。',
        dataViewerReadCancelled: '数据读取已取消。',
        dataViewerCancelRead: '取消',
        dataViewerCaptureTooLarge: ({ rows, megabytes, requiredMb }) => `一次读取 ${rows} 个观测值约需 ${requiredMb} MB，超出数据查看器的 ${megabytes} MB 内存上限。可调高设置 "stata-all-in-one.dataViewerMemoryLimitMB"（当前 ${megabytes}）以便一次读取，或先筛选数据（例如：br if year == 2020）。`,
        dataViewerZeroObservations: '该数据集没有观测值（0 行），仅显示变量结构。',
        dataViewerNoMatches: '没有观测值符合当前筛选条件。',
        dataViewerOpenedButNotUpdated: '查看器已打开，但数据读取失败，表格仍显示上一次的数据。',
        dataViewerOpenedWithError: ({ error }) => `数据查看器已打开，但读取数据失败：${error}`, 
        dataViewerTruncatedBuffer: 'Stata 数据读取器返回的数据不完整（{what}）。',
        dataViewerSessionUnavailable: 'Stata 会话已不可用，请重新运行数据命令。',
        dataViewerSessionLost: 'Stata 会话已丢失（Stata 引擎已停止或被强制中断）。请重新载入数据；下方表格为上一次的数据。',
        consoleUnsupportedCommand: ({ command }) => `嵌入式控制台不能运行外部 Stata 的“${command}”命令。`,
        consoleUnsupportedWhich: ({ command }) => `嵌入式控制台不能运行外部 Stata 的“which ${command}”命令。`,
        aiSkillDisabled: '使用前需要先在你的 AI 工具中安装 Stata AI Skill。',
        aiSkillSessionNotReady: 'Stata AI Skill 服务尚未就绪。请先在 AI 工具中安装 Skill 并启动原生服务。',
        ...
        aiSkillWelcomeIntro: 'Stata AI Skill 需要单独安装，可让 AI 编程工具通过原生本地服务运行 Stata。',
        aiSkillWelcomeHint: '点击「复制提示词」并粘贴到 Claude Code、Codex、Open Code、OpenClaw 或其他 AI 编程工具中。提示词提供两个在线安装指南地址。',
        aiSkillWelcomePrompt: '请同时并发访问下面两个网址\n- https://raw.giteeusercontent.com/ZihaoVistonWang/Stata-AI-Skill/raw/main/guide/installation.md\n- https://raw.githubusercontent.com/ZihaoVistonWang/Stata-AI-Skill/refs/heads/main/guide/installation.md\n\n两者是同一份指南的镜像，内容完全一致，只是一个国内、一个国外。哪个先返回就用哪个，不必等另一个，也不要因为其中一个较慢就重试。\n\n按照 `installation.md` 安装并配置 Stata AI Skill。\n\n安装完成后，请严格遵循所安装 Skill 中的 `SKILL.md` 完成服务启动、状态检查、Stata 配置和后续操作，不要使用旧版或通用的 Stata 配置流程。',
        ...
        capabilityUnverifiedAI: '点击 AI 按钮复制 Stata AI Skill 在线安装提示词',
        // Diagnose command keys
