...
const UI_TEXT = {
    en: {
        ...
        aiSkillWelcomePrompt: 'I have Stata All in One VS Code extension installed with AI Skill enabled. Please set yourself up to run Stata code:\n\n1. Find the extension: search under ~/.vscode/extensions/ for a folder matching *stata-all-in-one*\n2. Read <extension-folder>/skill/SKILL.md — it has the complete skill definition and API docs\n3. Install or register this skill using your platform\'s mechanism (skills directory, rules file, config, etc.). **IMPORTANT: the skill MUST be named "stata-all-in-one-skill" — do not rename it.**\n4. The Stata HTTP API is provided by the VS Code extension on the user host machine at http://127.0.0.1:19521\n   - GET /status → check if running\n   - POST /execute with {"code":"..."} → run code\n5. Use curl to call this localhost API. Do not import or call the extension\'s internal JavaScript/native modules directly; the supported interface is HTTP.\n6. Important for Codex, containers, dev containers, remote shells, or any sandboxed tool: your shell\'s 127.0.0.1 may be the sandbox, not the user host. First test /status with a 2s timeout. If it fails with connection refused, timeout, OFFLINE, or GUI commands such as open/code cannot run, treat it as a sandbox/host-boundary issue. Request host/unsandboxed command execution to run curl against http://127.0.0.1:19521, or ask the user to run the curl command in a host terminal. Do not repeatedly retry inside the sandbox.',
