...
- **原生本地服务**：安装完成后，AI Skill 通过其原生可执行文件和本地服务运行，**无需任何Python、Node.js等第三方环境**。
- **安装交给 AI 就行**：点击编辑器工具栏的 `AI` 按钮，复制提示词并粘贴到 AI 工具。提示词通过 Gitee 和 GitHub 链接到在线 `installation.md`。

### 2. 代码运行 (与Stata 交互)

- **平台支持**：本拓展原生支持与 **macOS** 和 **Windows** 上的 Stata 无缝集成。
- **两种运行模式**：
  - **嵌入式控制台**（默认模式）🛠️：现在可以在 VS Code 内直接运行并展示Stata输出啦！包括*命令结果*、*错误信息*、*命令行窗口*和*图形输出*，真正实现一站式体验。控制台运行结果中的文件路径，点击即可打开。
...
- **多场景执行策略**：
  ...
  - **单行运行**：当光标在普通代码行任意位置时，点击 ▶️ 按钮 或按 `Ctrl / ⌘ + D`，只执行当前行代码。
  - **选中运行**：当选中单行或多行代码时，点击 ▶️ 按钮 或按 `Ctrl / ⌘ + D`，执行选中行的代码。支持**模糊选中**——无需精确选中代码段的*首行*或*尾行*，系统会自动捕捉并运行选中内容涉及的全部行。
...
1. 优化嵌入式控制台🛠️
   ...
   - **数据查看器联动**
     ...
     - **按需刷新**：数据变化后，再次点击已打开的数据查看器tab，会刷新并恢复原有视口。
2. 注释增强
