...
- **两种运行模式**：
  - **嵌入式控制台**（默认模式）🛠️：现在可以在 VS Code 内直接运行并展示Stata输出啦！包括*命令结果*、*错误信息*、*命令行窗口*和*图形输出*，真正实现 IDE 一体化体验。控制台运行结果中的文件路径，点击即可打开。
...
- **更多顺手功能**：
  - **内置 `br` / `browse`**：在嵌入式控制台模式下，无论从代码编辑器还是 Console 命令输入框运行 `br` 或 `browse`，都会直接打开内置数据查看器。
