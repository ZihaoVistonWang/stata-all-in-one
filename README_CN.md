...
<p align="center">
   | <b>版本:</b><a href="https://github.com/ZihaoVistonWang/stata-all-in-one/releases"> 0.3.3</a> | <b>作者:</b> <a href="https://zihaowang.cn">王梓豪</a> | <b>Translate:</b>
...
- **即点即看**：默认辅助侧栏模式下，点击 VS Code 资源管理器中的 `.dta` 文件，即可在右侧 **SAiO** 视图内的`数据查看器`中打开数据集。
  - **变量信息**：变量表格显示变量名称、标签、类型等元数据。
  - **数据浏览**：支持行列动态加载，无需打开Stata，现在在Vs Code中就能轻松浏览大数据集。
- **右侧统一工作区**：**SAiO** 辅助侧栏通过顶部固定的`控制台`和`数据查看器`两个内部标签组织功能。运行代码时自动展开并切换到`控制台`，不会创建新的编辑器标签页。
...
1. **运行模式** (`stata-all-in-one.runMode`)
   - `secondarySidebar`（默认）：在右侧 **SAiO** 辅助侧栏中运行代码，并在同一视图内切换`控制台`和`数据查看器`。
   - `embeddedConsole`：保留原有 **控制台 | Stata All in One** 编辑器标签页体验。
