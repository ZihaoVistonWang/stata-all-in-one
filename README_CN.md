...
<h3 align="center"><b>一站式 Stata 体验：</b></br>运行代码 + 语法高亮 + 代码提示 + 智能大纲 + 数据查看 + AI Skill，<b>All in One</b>!</h3>
...
- **原生本地服务**：安装完成后，AI Skill 通过其原生可执行文件和本地服务运行，**无需任何Python等第三方环境**。
...
- **两种运行模式**：
  - **嵌入式控制台**（默认模式）🛠️：现在可以在 VS Code 内直接运行并展示Stata输出啦！包括*命令结果*、*错误信息*、*命令行窗口*和*图形输出*，真正实现一体化体验。控制台运行结果中的文件路径，点击即可打开。
...
- **多场景执行策略**：
  - **章节运行**：当光标在标题行（如 `** # 标题`）时，点击 ▶️ 按钮 或按 `Ctrl / ⌘ + D`，将执行从该标题起始至下一个同级或高级别标题前的所有代码（即整个章节）。
  - **单行运行**：当光标在普通代码行（选中）时，点击 ▶️ 按钮 或按 `Ctrl / ⌘ + D`，只执行当前行代码。
  - **选中运行**：当选中多行代码时，点击 ▶️ 按钮 或按 `Ctrl / ⌘ + D`，执行选中行的代码。支持**模糊选中**——无需精确选中代码段的*首行*或*尾行*，系统会自动捕捉并运行选中内容涉及的全部行。
...
- **多级大纲识别**：自动识别 `**#` 至 `**######` 格式的注释行，最高支持 _6 级层级标题_。
  - **快捷键**：`Ctrl / ⌘ + 1-6` 快速转换对应等级标题，`Ctrl / ⌘ + 0` 恢复为普通代码行。
...
- **跑完就看**：跑完代码后，可以立即在`控制台`面板中的`数据查看器`中查看上述结果。
- **数据过滤**：提供 Stata 风格的列过滤功能，支持快速定位感兴趣的数据子集。
- **更多顺手功能**：
  - **内置 `br` / `browse`**：在嵌入式控制台模式下，无论从代码编辑器还是控制台命令输入框运行 `br` / `browse`+Stata筛选语句，都会直接打开内置数据查看器。
  - **查看长文本**：单元格中的文字太长、显示不全时，将鼠标停留在该单元格上即可查看完整内容。
  - **双击复制**：双击单元格，即可将完整内容复制到剪贴板。

### 6. 高效分隔线与样式

- **快速插入**：支持多种符号，显著提升代码的可读性。
  - **标准分隔符**：通过 `Ctrl / ⌘ + [符号]` 快速插入分隔线：
    - `Ctrl / ⌘ + -` (短横线) | `Ctrl / ⌘ + =` (等号) | `Ctrl / ⌘ + Shift + 8` (星号)
  - **自定义分隔符**：
    - `Ctrl + Alt + S` (Windows) | `Ctrl + ⌘ + S` (macOS)，此处 **S** 代表 "**S**eparator"（分隔符）。
...
1. 优化嵌入式控制台🛠️
   ...
   - **大输出与运行控制**
     ...
     - **流畅浏览**：大量输出采用分页显示，保留完整历史，并可通过“跳转到底部”按钮快速返回最新结果。
     - **可靠停止**：点击 Stop 或按 Esc 会先尝试软中断；无响应时强制终止并自动重建 Stata 会话。
     - **稳定状态**：运行计时在视图切换后仍保持准确，临时 do 文件输入不再重复显示。
   - **结果管理**

     - **完整导出**：将控制台结果导出为 HTML、Markdown 或 Notebook 时，保留全部历史记录。
     - **HTML 导航**：导出的 HTML 提供代码预览、语法高亮和连续导航，便于快速定位每次运行。
     - **可靠文件链接**：识别并验证输出中的文件与目录路径，支持中文、跨行及重复路径，点击即可打开。
2. 注释增强

   - **一键切换**：使用 `Ctrl / ⌘ + /` 快速切换行注释状态。
   - **可选样式**：默认使用 `//`，支持在设置中更改为其他合法注释符。
3. 内置帮助

   - **悬浮帮助提示**：鼠标悬停在 Stata 命令上时显示帮助信息，并自动过滤掉一些非实用命令（如 `#delimit`、`using` 等）。
   - **输出帮助文本**：例如：选中 `regress`，按下快捷键`Ctrl / ⌘ + Shift + H`，外部应用（externalApp）模式会打开 Stata 的 `regress` 帮助页面，而嵌入式控制台（embeddedConsole）模式则会在控制台中直接显示 `regress` 的帮助文本。<a id="line-break"></a>
4. 智能换行
...
5. 编辑器右键便捷功能
   ...
   - **智能运行**：根据光标与选区显示“运行当前章节”“运行当前行”或“运行选中行”，并同步对应快捷键提示。
   - **快捷编辑**：右键即可设置标题级别、插入分隔线、居中标题文本或按当前样式快速注释。
   - **AI Skill**：检测到可用的 Stata 许可证时，可从右键菜单打开 Stata AI Skill。
6. 安全重命名模式
...
自动探测未找到 Stata 时，选择 `没有找到你的 Stata？直接在 Stata 中完成配置`。根据拓展提示的两个步骤，分别在Stata输入命令，即可便捷完成配置。
...
2. **Stata 版本（macOS）** (`stata-all-in-one.stataVersionOnMacOS`)
   ...
   - Stata 运行版本。配置为空时，扩展会在启动时优先自动探测，最长等待 3 秒；优先数字版本最高的安装，同版本按 `StataMP`、`StataSE`、`StataBE`、`StataIC` 排序。未找到时可直接从当前运行的 Stata 完成配置。初始化随后检查准确的 `.app` 路径、控制台 dylib 和 `stata.lic`，尽可能初始化内置控制台，并通过中心弹窗只汇报一次结果。
3. **Stata 路径（Windows）** (`stata-all-in-one.stataPathOnWindows`)

   - Stata 执行文件路径（例如 `C:\Program Files\Stata17\StataMP-64.exe`）。配置为空时，扩展会在启动时优先自动运行内置的 `scripts/discover_stata_windows.bat` 注册表探测脚本，最长等待 5 秒。该 BAT 脚本也可独立运行并生成 `stata-discovery-report.json`，用于问题排查。未找到时可直接从当前运行的 Stata 完成配置。初始化随后检查 EXE、控制台 DLL 和 `stata.lic`，尽可能初始化内置控制台，并通过中心弹窗只汇报一次结果。
...
6. **显示操作按钮** (`stata-all-in-one.showActionButtons`)

   - `true`（默认）：在编辑器标题栏显示“Bug 反馈”和“Stata AI Skill”按钮。
   - `false`：隐藏这两个按钮。
7. **显示打赏支持按钮** (`stata-all-in-one.showSponsorButton`)

   - `true`：在 Stata 编辑器标题栏显示“打赏支持”按钮。
   - `false`（默认）：隐藏该按钮。
8. **启用 Ctrl+Shift+D 作为运行快捷键** (`stata-all-in-one.enableCtrlShiftD`)

- `true`：使用 `Ctrl / ⌘+Shift+D` 作为运行代码的快捷键。
- `false`（默认）：使用默认的 `Ctrl / ⌘+D` 快捷键。
...
| 版本  | 更新内容                                                                                                    | 发布日期   |
| ----- | ----------------------------------------------------------------------------------------------------------- | ---------- |
| 0.3.8 | 新增 Stata 编辑器右键便捷菜单与嵌入式控制台“跳转到底部”；修复部分文件路径无法点击跳转和运行停止问题，并提升大输出响应与结果导出体验。 | 2026-07-26 |
